...
public class DetectLeaksWindow : EditorWindow
{
	...
	[MenuItem("LeakDetect/DetectLeaks Window")]
	static void Init()
	{
	...
	void ShowWindow ()
	{
		...
		_detector = new GameObject("Detector");
		DontDestroyOnLoad(_detector);

		_detector.AddComponent<DetectLeaks>();
	...
	void OnGUI()
	{
		...
		if (GUILayout.Button("Show Detail", GUILayout.Height(30f)))
			ShowWindow();
		if (GUILayout.Button("Hide Detail", GUILayout.Height(30f)))
			HideWindow();
